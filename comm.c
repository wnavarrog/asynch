...
{
	int i,j,m,n,sender,steps_to_transfer,curr_idx,parval,s,dim,flag,position,total_links,count,removed,order,num_times;
	unsigned int loc,l;
	double discont_time;
	RKSolutionNode* node;
	Link *current,*next,*prev;
	MPI_Status status;
	int num_dense = GlobalVars->num_dense;
...
{
	int i,j,m,n,steps_to_transfer,curr_idx,s,dim,flag,position,total_links,sender,count,removed,order,num_times;
	unsigned int loc,parval,l;
...
ConnData* CreateConnData(char* connectstring)
{
	...
	strcpy(conninfo->connectinfo,connectstring);
		...
		conninfo->submission_size = 16384;	//2^14
		conninfo->submission = (char*) malloc(conninfo->submission_size * sizeof(char));
		conninfo->submission[0] = '\0';
	#else
		//conninfo->submission_size = 0;
		//conninfo->submission = NULL;
	#endif
