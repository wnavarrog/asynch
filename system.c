...
void Destroy_Link(Link* link_i,unsigned int list_length,int rkd_flag,Forcing** forcings,UnivVars* GlobalVars)
{
	...
	iv_free(link_i->iparams);
	if(link_i->upstream != NULL)
	{
		for(i=0;i<link_i->numparents;i++)
			free(link_i->upstream[i]);
		free(link_i->upstream);
		free(link_i->numupstream);
	}
	...
	if(link_i->method != NULL)
	{
		...
		{
			if(forcings[i]->flag != 4 && forcings[i]->flag != 7 && link_i->forcing_buff[i] != NULL)
...
void Destroy_UnivVars(UnivVars* GlobalVars)
{
	...
	//free(GlobalVars->identifier);
	if(GlobalVars->rsv_filename)	free(GlobalVars->rsv_filename);
	//if(GlobalVars->halt_filename)	free(GlobalVars->halt_filename);
	...
	//if(GlobalVars->dump_location != NULL)	free(GlobalVars->dump_location);
	//if(GlobalVars->halt_filename != NULL)	free(GlobalVars->halt_filename);
	v_free(GlobalVars->global_params);
	free(GlobalVars->dense_indices);
