...
{
	unsigned int i,j,N = asynch->N,*assignments = asynch->assignments,dim = asynch->GlobalVars->dim;
	...
	for(i=0;i<N;i++)
	{
		...
		{
			array[i] = v_get(dim);
			for(j=0;j<dim;j++)	array[i]->ve[j] = values[i*dim+j];
		}
		else
			array[i] = NULL;
	}
...
int Asynch_Custom_Model_py(asynchsolver* asynch,void (*SetParamSizes)(UnivVars*,PyObject*),void (*Convert)(VEC*,unsigned int,void*),void (*Routines)(Link*,unsigned int,unsigned int,unsigned short int,void*),
	void (*Precalculations)(Link*,VEC*,VEC*,IVEC*,unsigned int,unsigned int,unsigned short int,unsigned int,void*),unsigned int (*InitializeEqs)(VEC*,VEC*,IVEC*,QVSData*,unsigned short int,VEC*,unsigned int,unsigned int,unsigned int,void*),PyObject* lib)
{
