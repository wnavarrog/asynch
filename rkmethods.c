...
{
	unsigned int start = GlobalVars->diff_start;
	...
	for(i=0;i<link_i->numparents;i++)
	{
		...
		if(currentp->list->tail->t == t)
			v_copy(currentp->list->tail->y_approx,temp_parent_approx[0][i]);
		else
		{
			...
			//v_copy(curr_node->y_approx,temp_parent_approx[0][i]);
			//for(l=0;l<currentp->method->s;l++)
			//	daxpy(timediff*currentp->method->b_theta->ve[l],curr_node->next->k[l],temp_parent_approx[0][i],1);
			...
			if(link_i->alg != NULL)	link_i->alg(temp_parent_approx[0][i],GlobalVars->global_params,currentp->params,currentp->qvs,curr_node->state,temp_parent_approx[0][i]);
		}
	}

	//Step a
	//d0 = vector_norminf(y0,start);
	d0 = norm_inf(y0,SC,start);
	link_i->f(t_0,y0,temp_parent_approx[0],link_i->numparents,GlobalVars->global_params,link_i->forcing_values,link_i->qvs,link_i->params,link_i->iparams,link_i->state,link_i->upstream,link_i->numupstream,fy0);
	d1 = norm_inf(fy0,SC,start);
	...
	v_copy(y0,y1);
	daxpy(h0,fy0,y1,start);

	link_i->f(t_0 + h0,y1,temp_parent_approx[0],link_i->numparents,GlobalVars->global_params,link_i->forcing_values,link_i->qvs,link_i->params,link_i->iparams,link_i->state,link_i->upstream,link_i->numupstream,fy1);

	//Step d
	v_sub(fy1,fy0,fy1,start);
	d2 = norm_inf(fy1,SC,start);
...
if(link_i->ID == 318213)
{
printf("j = %u, timediff = %f\n",j,timediff);
Print_Vector(temp_parent_approx[j][i]);
Print_Vector(curr_node[i]->y_approx);
for(l=0;l<currentp->method->s;l++)
	Print_Vector(curr_node[i]->next->k[l]);
getchar();
}
*/
		}
	...
	for(i=0;i<s;i++)
	{
		v_copy(y_0,sum);
		for(j=0;j<i;j++)
			daxpy(h*A->me[i][j],temp_k[j],sum,0);
		link_i->CheckConsistency(sum,params,GlobalVars->global_params);
		link_i->f(t + c->ve[i] * h,sum,temp_parent_approx[i],link_i->numparents,GlobalVars->global_params,link_i->forcing_values,link_i->qvs,params,link_i->iparams,link_i->state,link_i->upstream,link_i->numupstream,temp_k[i]);
	}

	//Build the solution
	v_copy(y_0,new_y);
	for(i=0;i<s;i++)	daxpy(h*b->ve[i],temp_k[i],new_y,0);
	...
	double err_1;
	v_copy(temp_k[0],sum);
	sv_mlt(h * e->ve[0],sum,0);
	for(i=1;i<s;i++)	daxpy(h*e->ve[i],temp_k[i],sum,0);
...
*/
		...
		store_k(temp_k,new_node->k,GlobalVars,s);
...
*/
				...
				//WriteStep(link_i->next_save,sum,GlobalVars,params,link_i->iparams,link_i->state,outputfile,link_i->output_user,&(link_i->pos));
				WriteStep(link_i->next_save,sum,GlobalVars,params,link_i->iparams,link_i->state,outputfile,link_i->output_user,&(link_i->pos_offset));
...
int ExplicitRKIndex1Solver(Link* link_i,UnivVars* GlobalVars,int* assignments,short int print_flag,FILE* outputfile,ConnData* conninfo,Forcing** forcings,TempStorage* workspace)
{
	...
	for(i=0;i<link_i->numparents;i++)
	{
		...
		for(j=0;j<s;j++)
		{
			...
			link_i->CheckConsistency(temp_parent_approx[j][i],currentp->params,GlobalVars->global_params);
			...
			link_i->alg(temp_parent_approx[j][i],GlobalVars->global_params,currentp->params,currentp->qvs,curr_node[i]->state,temp_parent_approx[j][i]);
		}
	}
	...
	for(i=0;i<s;i++)
	{
		v_copy(y_0,sum);
		for(j=0;j<i;j++)
			daxpy(h*A->me[i][j],temp_k[j],sum,1);
		link_i->CheckConsistency(sum,params,GlobalVars->global_params);
		link_i->f(t + c->ve[i] * h,sum,temp_parent_approx[i],link_i->numparents,GlobalVars->global_params,link_i->forcing_values,link_i->qvs,params,link_i->iparams,link_i->state,link_i->upstream,link_i->numupstream,temp_k[i]);
	}
	...
	//Build the solution
	v_copy(y_0,new_y);
	for(i=0;i<s;i++)	daxpy(h*b->ve[i],temp_k[i],new_y,1);
	//new_node->state = 0;
	new_node->state = link_i->state_check(new_y,GlobalVars->global_params,params,link_i->qvs,link_i->dam);
	link_i->CheckConsistency(new_y,params,GlobalVars->global_params);
	link_i->alg(new_y,GlobalVars->global_params,params,link_i->qvs,new_node->state,new_y);
	...
	double err_1;
	v_copy(temp_k[0],sum);
	sv_mlt(h * e->ve[0],sum,1);
	for(i=1;i<s;i++)	daxpy(h*e->ve[i],temp_k[i],sum,1);
	...
	err_1 = norm_inf(sum,temp,1);
	double value_1 = pow(1.0/err_1,1.0/meth->e_order);

	//Check the dense error (in inf norm) to determine if the step can be accepted
	double err_d;
	v_copy(temp_k[0],sum);
	sv_mlt(h * d->ve[0],sum,1);
	for(i=1;i<s;i++)	daxpy(h*d->ve[i],temp_k[i],sum,1);
	for(i=0;i<dim;i++)
		temp->ve[i] = max(fabs(new_y->ve[i]),fabs(y_0->ve[i])) * error->reltol_dense->ve[i] + error->abstol_dense->ve[i];
	err_d = norm_inf(sum,temp,1);
	double value_d = pow(1.0/err_d,1.0/meth->d_order);
...
*/
				...
				link_i->alg(sum,GlobalVars->global_params,link_i->params,link_i->qvs,link_i->list->tail->state,sum);
				link_i->CheckConsistency(sum,params,GlobalVars->global_params);
	...
	//if(err_1 < 1.0)
	{
		...
		else
		{
			...
			if(sign_l != sign_h)
			{
				...
				for(i=0;i<s;i++)	daxpy(h*meth->b_theta_deriv->ve[i],temp_k[i],sum,0);
				...
				for(i=0;i<link_i->numparents;i++)
				{
					...
					link_i->alg(temp_parent_approx[0][i],GlobalVars->global_params,currentp->params,currentp->qvs,curr_node[i]->next->state,temp_parent_approx[0][i]);
				}

				//Exact derivative at time t + h
				link_i->f(t + h,new_y,temp_parent_approx[0],link_i->numparents,GlobalVars->global_params,link_i->forcing_values,link_i->qvs,params,link_i->iparams,link_i->state,link_i->upstream,link_i->numupstream,temp);
				...
				do
				{
					...
					for(i=0;i<s;i++)	daxpy(h*meth->b_theta->ve[i],temp_k[i],sum,1);
					link_i->alg(sum,GlobalVars->global_params,link_i->params,link_i->qvs,new_node->state,sum);
		...
		else
		{
			...
			if(sign_l != sign_h)
			{
				...
				do
				{
					...
					current_theta = (xm - t)/h;
					meth->dense_b(current_theta,meth->b_theta);
					for(i=0;i<s;i++)	daxpy(h*meth->b_theta->ve[i],temp_k[i],sum,0);
					sign_m = link_i->state_check(sum,GlobalVars->global_params,params,link_i->qvs,link_i->dam);
...
//	MAT* JMatrix = workspace->JMatrix;
	...
	do
	{
		...
		for(j=0;j<s;j++)
		{
			...
			link_i->f(t+c->ve[j]*h,sum,temp_parent_approx[j],link_i->numparents,GlobalVars,link_i->forcing_values,link_i->qvs,params,link_i->iparams,link_i->state,link_i->upstream,link_i->numupstream,link_i->upstream,link_i->numupstream,temp);
			for(l=0;l<s;l++)
	...
	v_copy(sum,err);
	link_i->f(t,y_0,temp_parent_approx[s],link_i->numparents,GlobalVars,link_i->forcing_values,link_i->qvs,params,link_i->iparams,link_i->state,link_i->upstream,link_i->numupstream,temp);	//Compute the extra k
	...
	if(link_i->rejected == 0)	//If the previous step was rejected
	{
		...
		v_copy(sum,err);
		link_i->f(t,temp,temp_parent_approx[s],link_i->numparents,GlobalVars,link_i->forcing_values,link_i->qvs,params,link_i->iparams,link_i->state,link_i->upstream,link_i->numupstream,temp);
...
//	v_copy(sum,err);
//	link_i->f(t,y_0,temp_parent_approx[s],link_i->numparents,GlobalVars,link_i->forcing_values,link_i->qvs,params,link_i->iparams,link_i->upstream,link_i->numupstream,temp);	//Compute the extra k
...
//		v_add(y_0,err,temp,0);
//		v_copy(sum,err);
//		link_i->f(t,temp,temp_parent_approx[s],link_i->numparents,GlobalVars,link_i->forcing_values,link_i->qvs,params,link_i->iparams,link_i->upstream,link_i->numupstream,temp);
...
//	if(err_1 < 1.0 && err_d < 1.0)
	if(err_1 < 1.0)
	{
		...
		for(j=0;j<s;j++)
		{
			...
			if(dim > 3 && sum->ve[3] > 1.0 - sum->ve[2])	sum->ve[3] = 1.0 - sum->ve[2];
			link_i->f(t+c->ve[j]*h,sum,temp_parent_approx[j],link_i->numparents,GlobalVars,link_i->forcing_values,link_i->qvs,params,link_i->iparams,link_i->state,link_i->upstream,link_i->numupstream,k[j]);
		}
		link_i->last_t = t + h;
		link_i->current_iterations++;
		link_i->h_old = h;
	...
	if(print_flag)
	{
		while( t <= link_i->next_save && (link_i->next_save < link_i->last_t || fabs(link_i->next_save - link_i->last_t)/link_i->next_save < 1e-12) )
		{
			...
			if( change_value && fabs((link_i->next_save - link_i->last_t)/link_i->next_save ) < 1e-12 )
				WriteStep(link_i->next_save,new_y,GlobalVars,params,link_i->iparams,link_i->state,outputfile,link_i->output_user,&(link_i->pos_offset));
			else
				WriteStep(link_i->next_save,y_0,GlobalVars,params,link_i->iparams,link_i->state,outputfile,link_i->output_user,&(link_i->pos_offset));
			link_i->next_save += link_i->print_time;
		}
	}
...
void store_k(VEC** full_k,VEC** k,UnivVars* GlobalVars,unsigned int s)
{
	unsigned int i,j;
	unsigned int* dense_indices = GlobalVars->dense_indices;
	unsigned int num_dense = GlobalVars->num_dense;
...
void BackupParent(Link* currentp,double time,VEC** backup,UnivVars* GlobalVars)
{
	...
	for(l=0;l<currentp->method->s;l++)
		daxpy(timediff*currentp->method->b_theta->ve[l],curr_node->next->k[l],storage,GlobalVars->diff_start);

	//Build the algebraic variable
	if(currentp->alg != NULL)
		currentp->alg(storage,GlobalVars->global_params,currentp->params,currentp->qvs,curr_node->next->state,storage);
}
