...
void AsynchSolver(Link** sys,unsigned int N,unsigned int* my_sys,unsigned int my_N,unsigned int my_max_nodes,UnivVars* GlobalVars,int* assignments,unsigned int** id_to_loc,TempStorage* workspace,Forcing** forcings,ConnData** db_connections,TransData* my_data,short int print_flag,FILE* outputfile)
{
	...
	short int* done = (short int*) malloc(my_max_nodes*sizeof(short int));
	...
	for(i=0;i<GlobalVars->num_forcings;i++)
	{
		...
		{
			forcings[i]->passes = forcings[i]->GetPasses(forcings[i]);
	...
	for(k=0;k<passes;k++)
	{
		...
		curr_idx = 0;
		last_idx = my_N - 1;
		for(i=0;i<my_max_nodes;i++)	done[i] = 0;
...
//printf("setting forcing maxtime to %e\n",forcings[i]->maxtime);
					//(forcings[i]->iteration)++;
				}
		...
		if(GlobalVars->res_flag)
		{
			for(i=0;i<my_N;i++)	//!!!! Can we loop over just the links with reservoirs? Improve id_to_loc. !!!!
			{
				...
				{
					current->f(current->last_t,current->list->tail->y_approx,NULL,current->numparents,GlobalVars->global_params,current->forcing_values,current->qvs,current->params,current->iparams,current->state,current->upstream,current->numupstream,current->list->tail->y_approx);
...
void AsynchSolverPersis(Link** sys,unsigned int* my_sys,unsigned int my_N,unsigned int my_max_nodes,UnivVars* GlobalVars,int* assignments,TempStorage* workspace,Forcing** forcings,ConnData** db_connections,TransData* my_data,short int print_flag,FILE* outputfile)
{
	...
	unsigned int last_idx = my_N - 1;
	unsigned int around = 0;
	unsigned int two_my_N = 2*my_N;
	...
	for(k=0;k<passes;k++)
	{
		...
		for(i=0;i<GlobalVars->num_forcings;i++)
		{
			if( forcings[i]->flag != 5 && fabs(sys[my_sys[0]]->last_t - forcings[i]->maxtime) < 1e-14 )
			{
				forcings[i]->maxtime = forcings[i]->GetNextForcing(sys,N,my_sys,my_N,assignments,GlobalVars,forcings[i],db_connections,id_to_loc,i);
				(forcings[i]->iteration)++;
			}
			...
			else	//Compute an iteration
			{
				...
				if(current->current_iterations < GlobalVars->iter_limit)
				{
					...
					if(current->numparents == 0)	//Leaf
					{
						...
						{
							for(i=0;i<GlobalVars->num_forcings;i++)		//!!!! Put this in RKSolver !!!!
								if(current->last_t < current->forcing_change_times[i])	current->h = min(current->h,current->forcing_change_times[i] - current->last_t);
							current->rejected = current->RKSolver(current,GlobalVars,assignments,print_flag,outputfile,db_connections[ASYNCH_DB_LOC_HYDRO_OUTPUT],workspace);
						}
						...
						if(current->last_t + current->h >= maxtime  && current->current_iterations < GlobalVars->iter_limit && current->last_t < maxtime)	//If less than a full step is needed, just finish up
						{
							for(i=0;i<GlobalVars->num_forcings;i++)
								if(current->last_t < current->forcing_change_times[i])	current->h = min(current->h,current->forcing_change_times[i] - current->last_t);
							current->h = maxtime - current->last_t;
							current->rejected = current->RKSolver(current,GlobalVars,assignments,print_flag,outputfile,db_connections[ASYNCH_DB_LOC_HYDRO_OUTPUT],workspace);
	...
	fflush(stdout);
	Transfer_Data_Finish(my_data,sys,assignments,GlobalVars);
	if(my_rank == 0)	printf("%i: Going to next set of rain data.\n",my_rank);
